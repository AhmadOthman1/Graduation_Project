...
class MyPageCalendarController extends GetxController {
  ...
  onMoreOptionSelected(String option, int id,pageId) async {
    switch (option) {
      case 'Delete':
        return await deleteEvent(id,pageId);
        break;
    }
  }

  deleteEvent(id,pageId) async {
  ...
  addNewEvent(newAppointments,pageId) async {
    var url = "$urlStarter/user/addNewPageEvent";
    Map<String, dynamic> jsonData = {
      "pageId":pageId,
    ...
    if (response.statusCode == 403) {
      await getRefreshToken(GetStorage().read('refreshToken'));
      addNewEvent(newAppointments,pageId);
      return;
    } else if (response.statusCode == 401) {
      _logoutController.goTosigninpage();
    }
