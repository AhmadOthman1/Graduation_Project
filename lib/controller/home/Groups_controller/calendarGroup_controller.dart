...
class CalendarGroupController extends GetxController {
  ...
  onMoreOptionSelected(String option, int id,groupId) async {
    switch (option) {
      case 'Delete':
        return await deleteEvent(id,groupId);
        break;
    }
  }

  deleteEvent(id,groupId) async {
    var url = "$urlStarter/user/deleteGroupEvent";
  ...
  addNewEvent(newAppointments,groupId) async {
    var url = "$urlStarter/user/addNewGroupEvent";
    Map<String, dynamic> jsonData = {
      "groupId":groupId,
    ...
    if (response.statusCode == 403) {
      await getRefreshToken(GetStorage().read('refreshToken'));
      addNewEvent(newAppointments,groupId);
      return;
    } else if (response.statusCode == 401) {
      _logoutController.goTosigninpage();
    }
  ...
  Future<void> getEvents(groupid) async {
    var url = "$urlStarter/user/getGroupCalender?groupId=$groupid";
